ServerEvents.recipes(naluminaproject => {
    naluminaproject.shaped('kubejs:universal_press', [
        'FPF',
        'CSL',
        'FEF'
    ], {
        F: 'megacells:sky_steel_ingot',
        P: 'ae2:silicon_press',
        C: 'ae2:calculation_processor_press',
        S: 'minecraft:iron_block',
        L: 'ae2:logic_processor_press',
        E: 'ae2:engineering_processor_press'
    }).id('naluminaproject:universal_press')
    function universalPress(input, output, id) {
        naluminaproject.custom(
            {
                type: 'ae2:inscriber',
                ingredients: {
                    top: {
    ...
    function createCrystalAssemblerRecipe(output, inputs, id, fluid) {
        ...
        naluminaproject.custom(recipe).id(`kubejs:crystal_assembler/${id}`);
    }
    ...
    colors.forEach(color => {
        naluminaproject.shapeless(`4x ae2:${color}_covered_cable`, [`ae2:${color}_covered_dense_cable`]).id(`naluminaproject:ae2/${color}_dense_covered_to_normal`);
        naluminaproject.shapeless(`4x ae2:${color}_smart_cable`, [`ae2:${color}_smart_dense_cable`]).id(`naluminaproject:ae2/${color}_smart_covered_to_normal`);
        naluminaproject.shapeless(`ae2:${color}_covered_dense_cable`, [`4x ae2:${color}_covered_cable`]).id(`naluminaproject:ae2/${color}_covered_to_dense`);
